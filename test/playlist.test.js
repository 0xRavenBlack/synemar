...
function makeEngine() {
  ...
  function advance(ms) {
    ...
    return el => el._pauses;
  }
  return { els, engine, timers, advance, setTime: (t) => { time = t; } };
}
...

{
  ...
  assert.strictEqual(els[old]._pauses, oldPausesBefore + 1, 'stale old element is stopped once the crossfade elapses');
  assert.strictEqual(els[active]._pauses, activePausesBefore, 'active element is not stopped by the cleanup timer');
  assert.ok(els[active].src !== '', 'active element keeps its source (background is not wiped)');
  console.log('playlist: cleanup timer stops only the stale old element OK');
}
